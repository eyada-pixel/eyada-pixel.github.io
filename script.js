...
function App(){
  ...
  // === دوال جديدة للصورة والدوام فقط ===
  const handleTherapistImage=(e, isEdit=false)=>{
    const file=e.target.files[0];
    if(!file) return;
    if(file.size>900000){ showAlert("الصورة كبيرة - اختر اقل من 900KB","error"); return; }
  ...
  const saveEditTherapist=async()=>{
    ...
    try{
      ...
      await batch.commit();
      showAlert("✅ تم تعديل المعالج","success");
  ...
  return React.createElement("div", {className:"min-h-screen bg-[#f6f8fc]"},
    React.createElement("header", {className:"sticky top-0 z-30 bg-white border-b shadow-sm no-print"},
      React.createElement("div", {className:"max-w-[1280px] mx-auto px-4 h-16 flex items-center justify-between"},
        React.createElement("div", {className:"flex items-center gap-3"},
          ...
          React.createElement("div", {}, React.createElement("div", {className:"font-bold text-[15px]"}, "عيادة رقية للعلاج الطبيعي ✅"), React.createElement("div", {className:"text-[10px] text-emerald-600 font-bold"}, "تعديل + حذف + تضارب + دوام + صورة"))
        ),
    ...
    React.createElement("div", {className:"sticky top-16 z-20 bg-white border-b no-print"},
      React.createElement("div", {className:"max-w-[1280px] mx-auto px-4 flex gap-2 overflow-x-auto"},
        [{id:"today-sessions",label:"🕐 جلسات اليوم"},{id:"add-patient",label:"➕ إضافة مراجع"},{id:"patients-list",label:"👥 المراجعين - تعديل شامل"},{id:"therapists",label:"👨⚕ الكادر"},{id:"notifications",label:"🔔 الإشعارات"}].map(t=>React.createElement("button", {key:t.id,onClick:()=>setTab(t.id),className:`h-12 px-4 whitespace-nowrap border-b-2 text-[13px] font-medium ${tab===t.id?"border-b-blue-600 text-blue-600 bg-blue-50/50":"border-b-transparent text-slate-600"}`}, t.label))
      )
    ),
    ...
    React.createElement("main", {className:"max-w-[1280px] mx-auto p-4 pb-10"},
          ...
          React.createElement("div", {className:"overflow-x-auto"},
            React.createElement("table", {className:"w-full text-[13px]"},
              ...
              React.createElement("tbody", {},
                todaySessions.map(s=>React.createElement("tr", {key:s.id,className:"border-t hover:bg-blue-50/50"},
                  ...
                  React.createElement("td", {className:"p-3"}, React.createElement("select", {value:s.therapistName||"",onChange:(e)=>updateSession(s.id,{therapistName:e.target.value}),className:"text-[11px] px-2 py-1 rounded-full border font-bold"}, therapists.map(t=>React.createElement("option", {key:t.id,value:t.name}, t.name)))),
              ...
              React.createElement("div", {className:"flex justify-between items-start"},
                React.createElement("div", {className:"flex-1"},
                  ...
                  React.createElement("div", {className:"text-[12px] text-slate-600 mt-1 space-y-1"},
                    ...
                    React.createElement("div", {}, `📍 ${p.address||"بدون عنوان"} • 🩺 ${p.injury_type||"-"} • 👨⚕ ${p.therapistName||"-"}`)
                  ),
                  pSessions.length>0&&React.createElement("div", {className:"mt-3 bg-slate-50 rounded-xl p-3"},
                    React.createElement("div", {className:"text-[11px] font-bold mb-2"}, "📅 جلساته:"),
                    React.createElement("div", {className:"flex flex-wrap gap-1.5"},
                      pSessions.slice(0,8).map(s=>React.createElement("div", {key:s.id,className:`px-2 py-1 rounded-full text-[10px] font-bold border flex items-center gap-1 ${s.date===getTodayDate()?"bg-blue-600 text-white":s.status==="مكتملة"?"bg-emerald-100 text-emerald-700":s.status==="ملغاة"?"bg-red-100 text-red-700":"bg-white"}`},
                        `${s.date} ${s.time||""} - ${s.therapistName||""}`,
                        React.createElement("button", {onClick:()=>deleteSingleSession(s.id),className:"mr-1 text-[10px]"}, "×")
                      )),
                      pSessions.length>8&&React.createElement("div", {className:"text-[10px] text-slate-500"}, `+${pSessions.length-8} أخرى`)
                    )
                  )
                ),
        ...
        editingPatient&&React.createElement("div", {className:"fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"},
          React.createElement("div", {className:"bg-white rounded-[20px] p-6 w-full max-w-xl max-h-[92vh] overflow-y-auto"},
            React.createElement("div", {className:"flex justify-between items-center mb-4"},
              React.createElement("h3", {className:"font-bold text-[16px]"}, `✏ تعديل شامل: ${editingPatient.name}`),
              React.createElement("div", {className:"text-[11px] bg-amber-100 px-3 py-1 rounded-full"}, `${schedules.filter(s=>s.patientId===editingPatient.id||s.patientPhone===editingPatient.phone).length} جلسة ستتحدث`)
            ),
            React.createElement("div", {className:"space-y-3"},
              React.createElement("div", {className:"grid grid-cols-2 gap-2"},
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "الاسم *"), React.createElement("input", {value:editForm.name,onChange:(e)=>setEditForm({...editForm,name:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"})),
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "الموبايل *"), React.createElement("input", {value:editForm.phone,onChange:(e)=>setEditForm({...editForm,phone:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1",style:{direction:"ltr"}}))
              ),
              React.createElement("div", {className:"grid grid-cols-3 gap-2"},
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "العمر"), React.createElement("input", {type:"number",value:editForm.age_value,onChange:(e)=>setEditForm({...editForm,age_value:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"})),
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "الوحدة"), React.createElement("select", {value:editForm.age_unit,onChange:(e)=>setEditForm({...editForm,age_unit:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"}, React.createElement("option", {}, "سنة"), React.createElement("option", {}, "شهر"), React.createElement("option", {}, "أشهر"), React.createElement("option", {}, "يوم"), React.createElement("option", {}, "أيام"))),
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "الجنس"), React.createElement("select", {value:editForm.gender,onChange:(e)=>setEditForm({...editForm,gender:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"}, React.createElement("option", {}, "ذكر"), React.createElement("option", {}, "أنثى")))
              ),
              React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "العنوان - يظهر بكل مكان"), React.createElement("input", {value:editForm.address,onChange:(e)=>setEditForm({...editForm,address:e.target.value}),placeholder:"العنوان",className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"})),
              React.createElement("div", {className:"grid grid-cols-2 gap-2"},
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "نوع الإصابة"), React.createElement("select", {value:editForm.injury_type,onChange:(e)=>setEditForm({...editForm,injury_type:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1"}, React.createElement("option", {value:""}, "اختر..."), injuryTypes.map(t=>React.createElement("option", {key:t,value:t}, t)))),
                React.createElement("div", {}, React.createElement("label", {className:"text-[11px] font-bold"}, "المعالج - يحدث كل جلساته"), React.createElement("select", {value:editForm.therapistName,onChange:(e)=>setEditForm({...editForm,therapistName:e.target.value}),className:"w-full h-10 px-3 rounded-xl border text-[13px] mt-1 bg-amber-50"}, React.createElement("option", {value:""}, "اختر..."), therapists.map(t=>React.createElement("option", {key:t.id,value:t.name}, t.name))))
              ),
              React.createElement("div", {className:"bg-blue-50 border border-blue-200 rounded-xl p-3 text-[11px] text-blue-800"}, "💡 هذا التعديل راح يحدث الاسم والموبايل ونوع الإصابة والمعالج في كل الجلسات المرتبطة بهذا المراجع تلقائياً"),
              React.createElement("div", {className:"flex gap-2 mt-2"},
                React.createElement("button", {onClick:saveEditPatient,className:"flex-1 h-11 bg-blue-600 text-white rounded-xl font-bold"}, "💾 حفظ التعديل الشامل"),
              ...
              React.createElement("div", {className:"border-t pt-3 mt-3"},
                React.createElement("button", {onClick:()=>deletePatientCompletely(editingPatient),className:"w-full h-10 bg-red-50 text-red-600 border border-red-200 rounded-xl font-bold text-[12px]"}, "🗑 حذف المراجع وكل جلساته نهائياً")
              )
            )
          )
        )
      ...
      !loading&&tab==="add-patient"&&React.createElement("div", {className:"grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-4"},
        React.createElement("div", {className:"bg-white rounded-[20px] border p-6"},
          React.createElement("h2", {className:"font-bold mb-4"}, "➕ إضافة مريض مع فحص تضارب"),
          React.createElement("div", {className:"space-y-3"},
            ...
            React.createElement("div", {className:"grid grid-cols-3 gap-2"},
              React.createElement("input", {type:"number",value:newPatient.age_value,onChange:(e)=>setNewPatient({...newPatient,age_value:e.target.value}),placeholder:"العمر",className:"h-10 px-3 rounded-xl border text-[13px]"}),
              React.createElement("select", {value:newPatient.age_unit,onChange:(e)=>setNewPatient({...newPatient,age_unit:e.target.value}),className:"h-10 px-3 rounded-xl border text-[13px]"}, React.createElement("option", {}, "سنة"), React.createElement("option", {}, "شهر"), React.createElement("option", {}, "يوم")),
              React.createElement("select", {value:newPatient.gender,onChange:(e)=>setNewPatient({...newPatient,gender:e.target.value}),className:"h-10 px-3 rounded-xl border text-[13px]"}, React.createElement("option", {}, "ذكر"), React.createElement("option", {}, "أنثى"))
            ),
            ...
            React.createElement("div", {className:"grid grid-cols-2 gap-3"},
              ...
              React.createElement("select", {value:newPatient.therapist_name,onChange:(e)=>setNewPatient({...newPatient,therapist_name:e.target.value}),className:"h-10 px-3 rounded-xl border text-[13px] bg-amber-50 border-amber-300"}, React.createElement("option", {value:""}, "المعالج *"), therapists.map(t=>{
                const daysText = t.availableDays && t.availableDays.length>0 ? ` - ${t.availableDays.map(d=>daysMap[d].replace(" (عطلة)","").substring(0,2)).join("/")}` : "";
        ...
        React.createElement("div", {className:"bg-white rounded-[20px] border p-6 h-fit"},
          ...
          React.createElement("div", {className:"space-y-2"},
            daysOrder.map(day=>{
              const selTh = therapists.find(t=>t.name===newPatient.therapist_name);
              const isOff = selTh && selTh.availableDays && selTh.availableDays.length>0 && !selTh.availableDays.includes(day);
              return React.createElement("label", {key:day,className:`p-3 rounded-xl border-2 flex items-center gap-3 cursor-pointer ${selectedDays.includes(day)?"border-blue-600 bg-blue-50":"border-slate-200"} ${isOff?"opacity-50 bg-red-50 border-red-200":""}`},
                ...
                React.createElement("span", {className:"font-bold text-[13px] flex-1"}, daysMap[day], isOff?React.createElement("span", {className:"text-[10px] text-red-600 mr-2"}, " (عطلة المعالج)"):null),
          ...
          React.createElement("button", {onClick:handleAddPatient,className:"mt-6 w-full h-11 bg-blue-600 text-white rounded-xl font-bold"}, "✅ حفظ")
        )
      ),
      ...
      !loading&&tab==="therapists"&&React.createElement("div", {className:"space-y-4"},
        React.createElement("div", {className:"grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-4"},
          React.createElement("div", {className:"bg-white rounded-[20px] border p-5 h-fit"},
            React.createElement("h3", {className:"font-bold mb-4 text-[14px]"}, "➕ إضافة معالج جديد"),
            React.createElement("div", {className:"space-y-3"},
              ...
              React.createElement("input", {value:newTherapist.name,onChange:(e)=>setNewTherapist({...newTherapist,name:e.target.value}),placeholder:"اسم المعالج *",className:"w-full h-10 px-3 rounded-xl border text-[13px]"}),
              React.createElement("input", {value:newTherapist.phone,onChange:(e)=>setNewTherapist({...newTherapist,phone:e.target.value}),placeholder:"الموبايل",className:"w-full h-10 px-3 rounded-xl border text-[13px]",style:{direction:"ltr"}}),
              React.createElement("input", {value:newTherapist.specialization,onChange:(e)=>setNewTherapist({...newTherapist,specialization:e.target.value}),placeholder:"التخصص",className:"w-full h-10 px-3 rounded-xl border text-[13px]"}),
              ...
              React.createElement("button", {onClick:handleAddTherapist,className:"w-full h-11 bg-blue-600 text-white rounded-xl font-bold"}, "✅ إضافة")
            )
          ),
          React.createElement("div", {className:"bg-white rounded-[20px] border p-5"},
            React.createElement("div", {className:"flex justify-between items-center mb-4"},
              React.createElement("h3", {className:"font-bold text-[14px]"}, `👨⚕ الكادر (${therapists.length})`),
              React.createElement("input", {value:searchTherapist,onChange:(e)=>setSearchTherapist(e.target.value),placeholder:"🔍 بحث...",className:"w-24 h-8 px-3 rounded-full border text-[11px]"})
            ),
            React.createElement("div", {className:"space-y-2"},
              therapists.filter(t=>!searchTherapist||t.name.includes(searchTherapist)).map(t=>React.createElement("div", {key:t.id,className:"p-4 rounded-2xl border flex gap-3 items-start"},
                t.image ? React.createElement("img", {src:t.image,className:"w-11 h-11 rounded-full object-cover"}) : React.createElement("div", {className:"w-11 h-11 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold"}, t.name.charAt(0)),
                React.createElement("div", {className:"flex-1"},
                  React.createElement("div", {className:"font-bold text-[13px]"}, t.name),
                  React.createElement("div", {className:"text-[11px] text-slate-500"}, `${t.phone||""} ${t.specialization? "• "+t.specialization:""}`),
                  t.availableDays&&t.availableDays.length>0&&React.createElement("div", {className:"flex gap-1 mt-1.5 flex-wrap"}, t.availableDays.map(d=>React.createElement("span", {key:d,className:"text-[9px] px-2 py-0.5 rounded-full bg-slate-900 text-white"}, daysMap[d].replace(" (عطلة)",""))))
                ),
                React.createElement("div", {className:"flex gap-1"},
                  React.createElement("button", {onClick:()=>openEditTherapist(t),className:"w-8 h-8 bg-blue-50 text-blue-600 rounded-full"}, "✏"),
                  React.createElement("button", {onClick:async()=>{ if(confirm("حذف؟")){ await db.collection("therapists").doc(t.id).delete(); fetchData(); }},className:"w-8 h-8 bg-red-50 text-red-600 rounded-full"}, "🗑")
                )
              ))
            )
          )
        ...
        editingTherapist&&React.createElement("div", {className:"fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"},
          React.createElement("div", {className:"bg-white rounded-[20px] p-6 w-full max-w-md max-h-[90vh] overflow-y-auto"},
            ...
            React.createElement("div", {className:"space-y-3"},
              React.createElement("div", {className:"flex justify-center"},
                React.createElement("div", {className:"relative"},
                  editTherapistForm.image ? React.createElement("img", {src:editTherapistForm.image,className:"w-20 h-20 rounded-full object-cover border-4 border-blue-100"}) : React.createElement("div", {className:"w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center text-2xl"}, "📷"),
                  React.createElement("label", {className:"absolute -bottom-1 -right-1 w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-white cursor-pointer"}, "✏", React.createElement("input", {type:"file",accept:"image/*",onChange:(e)=>handleTherapistImage(e,true),className:"hidden"}))
                )
              ),
      ...
      !loading&&tab==="notifications"&&React.createElement("div", {className:"bg-white rounded-[20px] border p-6"},
        React.createElement("h2", {className:"font-bold mb-4"}, "🔔 إرسال إشعار مباشر"),
        React.createElement("input", {value:oneSignalRestKey,onChange:(e)=>{ setOneSignalRestKey(e.target.value); localStorage.setItem("onesignal_rest_key", e.target.value); },placeholder:"REST API Key",type:"password",className:"w-full h-11 px-4 rounded-xl border text-[13px] mb-3",style:{direction:"ltr"}}),
        ...
        React.createElement("button", {onClick:async()=>{
          ...
          if(!oneSignalRestKey){ showAlert("ضع REST API Key","error"); return; }
          try{
            const res=await fetch("https://api.onesignal.com/notifications",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Basic ${oneSignalRestKey}`},body:JSON.stringify({app_id:"06265525-0c5a-43c1-8825-2e245ec16a05",included_segments:["Subscribed Users"],headings:{en:notifTitle},contents:{en:notifMsg}})});
            const d=await res.json(); if(d.id){ showAlert(`✅ تم الإرسال لـ ${d.recipients} مشترك`,"success"); } else throw new Error();
          }catch(e){ showAlert("❌ فشل - تأكد من المفتاح","error"); }
